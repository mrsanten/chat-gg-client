...
interface Props {
  ...
  avatar?: string;
  /** Klik w panel użytkownika (nowy układ) otwiera dialog profilu. */
  onOpenProfile?: () => void;
  ...
  /** Klik w ramkę avatara (klasyczny układ) — rodzic odpala file picker. */
  onChangeAvatar?: () => void;
...
export function Sidebar(props: Props) {
  ...
  // Wersja apki — panel użytkownika (nowy układ) / footer (classic).
  const [version, setVersion] = useState<string | null>(null);
  // Zegar — tylko classic (mobilny footer). Update co 30s wystarczy.
  ...
  const displayNick = nick && nick.trim().length > 0 ? nick : "Użytkownik";
  const presenceLabel =
    presence === "online"
      ? "Dostępny"
      : presence === "afk"
          ...
          <>
            <div className="gg-profile">
              ...
              <div className="gg-profile-info">
                <div className="gg-profile-name">{displayNick}</div>
                <div
                  className={`gg-profile-status gg-profile-status--${presence ?? "logged_out"}`}
                >
                  {presenceLabel}
                </div>
                <input
          ...
          <>
            ...
            >
              ...
              <span className="gg-userpanel-text">
                <span className="gg-userpanel-name">{displayNick}</span>
                <span
                  className={`gg-userpanel-status gg-userpanel-status--${presence ?? "logged_out"}`}
                >
                  {presenceLabel}
                </span>
              </span>
